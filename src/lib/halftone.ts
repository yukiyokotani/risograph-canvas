 ...
 * DOT_MAX_R … 濃度 1（最暗）での最大半径（×cell）。1/√2≈0.707 でセルを完全被覆（ベタ）。
 *   リソグラフは大面積のベタが苦手（ムラ・裏写り・乾き・紙の波打ち）で、最暗部でも
 *   網点を残すのが定石。そのため 100% ベタにはせず 0.58（≒被覆率 93%）で止め、
 *   最暗部にもわずかな網点の隙間を残す。d（＝密度マップ×Density）がこの上限へ向けて
 *   被覆率を駆動する（Density を上げると最暗部が 93% まで詰まる）。
 */
