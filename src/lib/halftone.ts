 ...
 * DOT_MAX_R … 濃度 1（最暗）での最大半径（×cell）。1/√2≈0.707 でセルを完全被覆
 *   （ベタ）。0.72 は確実にベタへ融合させるための僅かな上乗せ。
 *   これにより最大被覆率は人工的なキャップを設けず、濃度 d（＝密度マップ×Density）が
 *   そのまま駆動する。Density を上げれば最暗部が 100%（ベタ）まで到達し、
 *   下げれば網点の隙間が残る。
 */
const DOT_FILL_D = Math.PI / 4; // ≒0.785
const DOT_MAX_R = 0.72;
        ...
        for (let sxi = 0; sxi < SS; sxi++) {
          ...
          for (let dy = -1; dy <= 1 && !inside; dy++) {
            for (let dx = -1; dx <= 1; dx++) {
              ...
              // 100%（ベタ）まで太らせる（最大被覆率は d＝密度マップ×Density が駆動）。
              let radius: number;
              if (d <= DOT_FILL_D) {
